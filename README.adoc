...
**1.** Download Gradle 2.3 from the {Gradle_Downloads}[Gradle Downloads] page.

NOTE: Only the binaries are required, so look for the link to `gradle-2.3-bin.zip`. Alternatively, you can choose `gradle-2.3-all.zip` to download the sources and documentation as well as the binaries.
...
Help tasks
----------
components - Displays the components produced by root project 'complete'. [incubating]
...
Within the `dependencies` block, you declare a single dependency for Joda Time. Specifically, you are asking for (reading right to left) version 2.7 of the joda-time library, in the joda-time group. 
